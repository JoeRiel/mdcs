;;; mds-tq.el --- utility to maintain a transaction queue
...
;; A transaction queue may be created by calling `mds-tq-create'.

;; A request may be added to the queue by calling `mds-tq-enqueue'.  If
...
;; was provided in the call to `mds-tq-enqueue'.  Then we call the
...
(defun mds-tq-queue               (tq) (car tq))
(defun mds-tq-process             (tq) (car (cdr tq)))
(defun mds-tq-buffer              (tq) (cdr (cdr tq)))

;; The structure of `queue' is as follows
;; ((question regexp closure . fn)
;;  <other queue entries>)
;; question: string to send to the process
(defun mds-tq-queue-head-question (tq) (car (car (mds-tq-queue tq))))
;; regexp: regular expression that matches the end of a response from
;; the process
(defun mds-tq-queue-head-regexp   (tq) (car (cdr (car (mds-tq-queue tq)))))
;; closure: additional data to pass to the function
(defun mds-tq-queue-head-closure  (tq) (car (cdr (cdr (car (mds-tq-queue tq))))))
;; fn: function to call upon receiving a complete response from the
;; process
(defun mds-tq-queue-head-fn       (tq) (cdr (cdr (cdr (car (mds-tq-queue tq))))))

;; Determine whether queue is empty
(defun mds-tq-queue-empty         (tq) (not (mds-tq-queue tq)))

;;; Core functionality

;;;###autoload
(defun mds-tq-create (process)
...
to a tcp server on another machine."
  (let ((tq (cons nil (cons process
    ...
    (buffer-disable-undo (mds-tq-buffer tq))
    (set-process-filter process
			`(lambda (proc string)
			   (mds-tq-filter ',tq string)))
    tq))

(defun mds-tq-queue-add (tq question re closure fn)
  (setcar tq (nconc (mds-tq-queue tq)
		    (cons (cons question (cons re (cons closure fn))) nil)))
  'ok)

(defun mds-tq-queue-pop (tq)
  (setcar tq (cdr (car tq)))
  (let ((question (mds-tq-queue-head-question tq)))
    (condition-case nil
	(process-send-string (mds-tq-process tq) question)
      (error nil)))
  (null (car tq)))

(defun mds-tq-enqueue (tq question regexp closure fn &optional delay-question)
...
This produces more reliable results with some processes."
  (let ((sendp (or (not delay-question)
		   (not (mds-tq-queue tq)))))
    (mds-tq-queue-add tq (unless sendp question) regexp closure fn)
    (when sendp
      (process-send-string (mds-tq-process tq) question))))

(defun mds-tq-close (tq)
  "Shut down transaction queue TQ, terminating the process."
  (delete-process (mds-tq-process tq))
  (kill-buffer (mds-tq-buffer tq)))

(defun mds-tq-filter (tq string)
  "Append STRING to the TQ's buffer; then process the new data."
  (let ((buffer (mds-tq-buffer tq)))
...
(defun mds-tq-process-buffer (tq)
  ...
  (let ((buffer (mds-tq-buffer tq)))
    (when (buffer-live-p buffer)
      (set-buffer buffer)
      (if (= 0 (buffer-size)) ()
	(if (mds-tq-queue-empty tq)
	    (let ((buf (generate-new-buffer "*spurious*")))
	      (copy-to-buffer buf (point-min) (point-max))
	      (delete-region (point-min) (point))
	      (pop-to-buffer buf nil)
	      (error "Spurious communication from process %s, see buffer %s"
		     (process-name (mds-tq-process tq))
		     (buffer-name buf)))
	  (goto-char (point-min))
	  (if (re-search-forward (mds-tq-queue-head-regexp tq) nil t)
	      (let ((answer (buffer-substring (point-min) (point))))
		(delete-region (point-min) (point))
		(unwind-protect
		    (condition-case nil
			(funcall (mds-tq-queue-head-fn tq)
				 (mds-tq-queue-head-closure tq)
				 answer)
		      (error nil))
		  (mds-tq-queue-pop tq))
		(mds-tq-process-buffer tq))))))))
