...
(defun mds-output-goto-line (button)
  (save-excursion ;; FIXME: why:
    ...
    (when (looking-at mds-output-prompt-re)
	...
	(if fnd
	    (let ((procname (match-string-no-properties 0)))
	      (mds-showstat-view-dead-proc procname "0" state)))))))
  ...
  (save-excursion
	...
	(let ((state (match-string-no-properties 1)))
	  (if (string= state "")
	    ...
	    (mds-output-display-proc (mds-output-get-enclosing-procname) "0" state))))))
